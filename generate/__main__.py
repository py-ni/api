...
CONFLICTING_C_KEYWORDS = frozenset(('default', ))

def enum_name(enum_cls, namespace):
    return namespace + enum_cls.__name__
...
def generate_module(mod, action):
...
def generate_typedefs(name, obj, namespace):
    ...
    if issubclass(cls, Enum):
        ...
        if not is_namespace(cls):
            print(f"typedef struct Py{name}Ref Py{name}Ref;")
        for fname, item in cls.__dict__.items():
            if not fname.startswith("_"):
                generate_typedefs(fname, item, cls.__name__)
...
def generate_api(name, obj, namespace):
    ...
    if obj is Enum:
        return
    if isinstance(obj, FunctionType):
        if not is_function_pointer(obj):
    ...
    if issubclass(cls, Enum):
        print(f"enum {namespace}{cls.__name__} {{")
        for item in cls:
        ...
        if not is_namespace(cls):
            print(f"struct Py{name}Ref {{intptr_t _bits;}};")
        for fname, item in cls.__dict__.items():
            if not fname.startswith("_"):
...
definition_modules = [objects, vm, types_and_modules]

for mod in definition_modules:
    print(f"\n/* typedefs for {mod.__name__} */")
    generate_module(mod, generate_typedefs)

for mod in definition_modules:
    generate_module(mod, generate_api)
